...
def cross_validation(data, K, epoch, class_num, batch_size):
    ...
    for i in range(0,K):
        ...
        record = open('records_incep.txt', 'a+')
        record.write("%d fold\n" % i)
...
def Network_config(class_num=4, epoch=200, initial_epoch=0, batch_size=32,
    ...
    tools.create_directory('./tmpinception_incep/')
    weights_file = './tmpinception_incep/' + str(fold)+'-weights.{epoch:02d}-{categorical_accuracy:.4f}-{val_loss:.4f}-{val_categorical_accuracy:.4f}.h5'
    csv_file = './tmpinception_incep/record.csv'
    ...
    tensorboard = TensorBoard(log_dir='./logs_incep/', histogram_freq=0, batch_size=8, write_graph=True,
    ...
    print(confusion)
    f = open('confusion_matrix_incep.txt','a+')
...
def main():
    data = creat_list('/home/bell-chuangxingang/Data/HIE/DL4ETI-realcode/CreateRandomList/four_random_list.txt')
    cross_validation(data, K=10, epoch=50, class_num=4, batch_size=16)
main()