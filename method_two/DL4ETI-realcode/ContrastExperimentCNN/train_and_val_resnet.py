...
def Network_config(class_num=4, epoch=200, initial_epoch=0, batch_size=32,
    ...
    tools.create_directory('./tmpresnet_res/')
    weights_file = './tmpresnet_res/' + str(fold)+'-weights.{epoch:02d}-{categorical_accuracy:.4f}-{val_loss:.4f}-{val_categorical_accuracy:.4f}.h5'
    csv_file = './tmpresnet_res/record.csv'
    ...
    tensorboard = TensorBoard(log_dir='./logs_res/', histogram_freq=0, batch_size=8, write_graph=True,
    ...
    print(confusion)
    f = open('confusion_matrix_res.txt','a+')
